...
if (isset($_POST['submit'])) {
    ...
    if (move_uploaded_file($fileTmp, $filePath)) {
        $stmt = $con->prepare("INSERT INTO solutions (name, faculty, subject, course_type, province, year, file_path, description) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
        $stmt->bind_param("ssssssss", $name, $faculty, $subject, $course_type, $province, $year, $filePath, $description);
        if ($stmt->execute()) {
            echo "<div class='alert alert-success'>Solution uploaded successfully.</div>";
        } else {
            echo "<div class='alert alert-error'>Database error: " . $stmt->error . "</div>";
        }
        $stmt->close();
    } else {
...
<head>
    ...
    <title>Upload Solutions - Admin Dashboard</title>
...
<body>
    ...
    <main class="main-content">
        <div class="container">
            <div class="page-header">
                <h1 class="page-title">Upload Solutions</h1>
                <p class="page-subtitle">Share educational solutions in PDF format</p>
            </div>
                ...
                <form method="post" enctype="multipart/form-data" class="upload-form">
                    <div class="form-grid">
                        <div class="form-group">
                            <label class="form-label" for="name">Solution Title</label>
                            <input type="text" id="name" name="name" class="form-input" placeholder="Enter solution title" required>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <input type="number" id="year" name="year" class="form-input" placeholder="e.g., 2081" required>
                        </div>

                        <div class="form-group">
                            <label class="form-label" for="description">Description</label>
                            <textarea id="description" name="description" class="form-textarea" placeholder="Optional description about the solution"></textarea>
                        </div>

                        <div class="form-group">
                            <label class="form-label">Solution File (PDF)</label>
                            <div class="file-input-wrapper">
                                <input type="file" id="solution_file" name="solution_file" class="file-input" accept="application/pdf" required>
                                <label for="solution_file" class="file-input-label">
                                    <svg class="file-icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                                    ...
                                    <span id="file-label">Choose PDF file or drag and drop</span>
                                </label>
                            </div>
                        </div>
    ...
    <script>
        ...
        const fileInput = document.getElementById('solution_file');
        ...
        fileInput.addEventListener('change', function() {
            if (this.files && this.files.length > 0) {
                ...
                fileLabel.textContent = 'Choose PDF file or drag and drop';
            }
        });
